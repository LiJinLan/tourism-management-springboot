...
//    			}
    		...
    		*/
    		...
    		if( key == "orderBy") {
    			...
    			if(LineRecomStr != "") {
    				...
    				switch( String.valueOf(choiceObj.get(key))) {
	    				case "销量" :
	    					LineRecomStr = LineRecomStr + " ORDER BY tradeVolume +0 DESC";
	    					break;
	    				case "综合" :
	    					LineRecomStr = LineRecomStr + " ORDER BY satisfaction +0 DESC";
	    					break;
	    				default :
	    					System.out.println("不确定的排序");
    				}
    				/*Map<String, Object > queryLineRecObj = new HashMap<String, Object>();
    				String sqlOrderByStr = "select * from line where ";
    				...
    				switch( String.valueOf(choiceObj.get(key))) {
	    				case "销量" :
	    					LineRecomStr ="ORDER BY tradeVolume +0 DESC";
	    					break;
	    				case "综合" :
	    					LineRecomStr ="ORDER BY satisfaction +0 DESC";
	    					break;
	    				default :
	    					System.out.println("不确定的排序");
    				}
    	...
    	Map<String, Object > queryLineRecObj = new HashMap<String, Object>();
    	String sqlStr = "select * from line where ";
    	String xml_Sql = sqlStr + LineRecomStr ;	//将查询条件和查询语句拼接成字符串，放到对象中，查询方法传参数为对象，xml中只要${属性名}即可拿到组装成的Sql语句
    	queryLineRecObj.put("EXE_SQL", xml_Sql);
    	
    	if(flag){
    		//将满足出行方式，价格，城市条件的数据放到lineQueryList数组
    		List<Map<String, Object>> lineQueryList = this.lineDao.queryLineRecommend( queryLineRecObj );
    		
    		String lineDaysStr = (String)choiceObj.get("lineDays");
    		if( lineDaysStr != null) {
    			Map<String, Object> dayObj = new HashMap<String, Object>();
        		//数组中每个lineId放到对象dayObj中的属性，属性值为空数组
        		for(int i = 0; i< lineQueryList.size(); i++) {
        			Map<String, Object> item = lineQueryList.get(i);  		 
        			String listId = (String)item.get("id");
        			List<Map<String, Object>> lineDayList = new ArrayList<Map<String, Object>>();
        			dayObj.put(listId, lineDayList);
        			
        		}
        		//查询行程表中的所有数据放数组中
        		List<Map<String, Object>> lineScheduleAll = this.lineDao.queryLineScheduleAll();
        		for(int i = 0;i < lineScheduleAll.size(); i++) {
        			Map<String, Object> item = lineScheduleAll.get(i);
        			String lineDayId = (String)item.get("lineId");
        			
        			//只
        			List<Map<String,Object>> list =(List<Map<String,Object>>)dayObj.get(lineDayId);
        			if(list != null) {  //不为null，即在前面三大条件查出来的路线有值。
        				list.add(item);
        			}			
        		}
        			...
        			int lineDay = Integer.parseInt(lineDaysStr);			
        			List<Map<String, Object>> returnLineList = new ArrayList<Map<String, Object>>();
        			
            		for(int i = 0; i < lineQueryList.size(); i++ ) {
            			Map<String, Object> item = lineQueryList.get(i);
            			String lineId = (String)item.get("id");
            			List< Map<String, Object > > dayObjItem = (List< Map<String, Object > >) dayObj.get(lineId);   			
            			if( dayObjItem.size() == lineDay) {
            				returnLineList.add(item);    				
            			}  			 			
            		}

        		return returnLineList;
    		} else {
    			return lineQueryList;
    		}
    		...
    		String lineDaysStr = (String)choiceObj.get("lineDays");
    		if( lineDaysStr != null) {
    			Map<String, Object > queryLineRecObj2 = new HashMap<String, Object>();
    			int lineDay = Integer.parseInt(lineDaysStr);
    	    	String sqlStr2 = "select * from line where lineDays=" +lineDay ;
    	    	queryLineRecObj.put("EXE_SQL", sqlStr2);
    	    	return this.lineDao.queryLineRecommend( queryLineRecObj );
    		} else{
    			return this.lineDao.queryLineAll();
    		}
	...
	public int getDaysBefore(){
		return this.lineDao.getDaysBefore();
	}
	
	public List<Map<String, Object>> queryOrderRecom(Map<String, Object> sqlMap){
		return this.lineDao.queryOrderRecom(sqlMap);
	}
	
	public List<Map<String, Object>> queryOtherRecom(Map<String, Object> querySQLMap){
		//return this.lineDao.queryOtherRecom(querySQLMap);
		return this.lineDao.queryOtherRecom(querySQLMap);
	}
